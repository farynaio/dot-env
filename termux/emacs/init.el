...
(use-package xclip
  :defer 2
  :commands (yank kill-ring-save kill-region kill-line)
  :bind (("C-M-w" . kill-ring-save))
  :config
  (xclip-mode 1))
...
(use-package openwith
  ...
  (setq openwith-associations
        (list
         (list (openwith-make-extension-regexp '("pdf")) "okular" '(file))
         (list (openwith-make-extension-regexp '("doc" "docx" "odt" "ods" "odp" "xls" "xlsx" "ppt" "pptx")) "libreoffice" '(file))))

  (unless (executable-find "okular")
    (warn "'okular' not found, pdf files can't be viewed!"))
...
(use-package consult
  ...
  (defun my/switch-to-buffer ()
    (interactive)
    (if (or (tramp-file-name-p (buffer-file-name)) (and (derived-mode-p 'dired-mode)
     (file-remote-p default-directory)) (string-prefix-p "*debug tramp" (buffer-name)))
  ...
  (defun my/consult-ripgrep ()
    ...
    (let ((dir (if (projectile-project-p) (projectile-project-root) (read-directory-name "Dir: " nil)))
          (symbol (unless (derived-mode-p 'dired-mode) (thing-at-point 'symbol))))
...
(use-package tab-bar
  ...
  :bind (("C-x <left>" . tab-bar-switch-to-prev-tab)
         ...
         ("C-c 1" . (lambda () (interactive) (my/tab-bar-select-or-create 1)))
         ("C-c 2" . (lambda () (interactive) (my/tab-bar-select-or-create 2)))
         ("C-c 3" . (lambda () (interactive) (my/tab-bar-select-or-create 3)))
         ("C-c 4" . (lambda () (interactive) (my/tab-bar-select-or-create 4)))
         ("C-c 5" . (lambda () (interactive) (my/tab-bar-select-or-create 5)))
         ("C-c 6" . (lambda () (interactive) (my/tab-bar-select-or-create 6)))
         ("C-c 7" . (lambda () (interactive) (my/tab-bar-select-or-create 7)))
         ("C-c 8" . (lambda () (interactive) (my/tab-bar-select-or-create 8)))
         ("C-c 9" . (lambda () (interactive) (my/tab-bar-select-or-create 9)))
         ("C-c 0" . (lambda () (interactive) (my/tab-bar-select-or-create 10))))
  :custom
  ...
  (defun my/tab-bar-tab-name ()
    "Use TITLE org file property as tab name when in an org buffer, or use other rules if given, else fallback to 'tab-bar-tab-name-current'."
...
(defun my/html-inside-tag-p ()
  "Return t if point is inside an HTML/XML tag."
  (save-excursion
    (let ((end (point)))
      (and (search-backward "<" nil t)
           (not (search-forward ">" end t))))))

(defun my/jump-matching (&optional arg)
  "Jumps between matching parens, brackets and HTML tag open / entry based on cursor position."
  (interactive "P")
  (cond
   ...
   ((looking-at "\\s]") (backward-list 1))
   ((looking-back "\\s]") (backward-list 1))
   ((and (derived-mode-p 'html-mode 'nxml-mode)
         (or (looking-at "<")
             (my/html-inside-tag-p)
             (and (= (char-before) ?>)
                  (save-excursion
                    (backward-char 1)
...
(if (boundp 'my/epa-file-encrypt-to-default)
    (use-package epa
      ...
      (add-hook 'find-file-hook #'my/epa-find-file-hook))
...
(use-package dired
  ...
  :config
  (defun my/dired-unmark-after-copy ()
    "Unmark all files after copy operation."
    (when (derived-mode-p 'dired-mode)
      (dired-unmark-all-marks)))
  (advice-add 'dired-do-copy :after #'my/dired-unmark-after-copy)
  ...
  (defun my/dired-marked-count ()
    (when (and (derived-mode-p 'dired-mode)
               (memq last-command
                     '(dired-mark dired-unmark dired-mark-subtree
                                  dired-mark-files-regexp dired-mark-if dired-mark-sexp)))
      (message "%d marked" (length (dired-get-marked-files nil)))))
  (add-hook 'post-command-hook #'my/dired-marked-count)
...
(use-package major-mode-hydra
  :defer 1
  :commands (major-mode-hydra hydra-base/body hydra-tab-bar/body)
  :bind (("C-f" . hydra-base/body)
         ("C-t" . hydra-tab-bar/body)
         ("C-r" . major-mode-hydra))
  :config
  ...
  (defun my/image-slicing-mode-toggle ()
    (interactive)
    (setq-local image-slicing-max-width (window-width nil t) * 0.6)
    (image-slicing-mode (if image-slicing-mode -1 1))
    (if image-slicing-mode
        (message "image-slicing-mode enabled")
      (message "image-slicing-mode disabled")))
  ...
  (pretty-hydra-define hydra-tab-bar
    ...
    ("Action"
     ...
     "Jump to"
     (("1" (my/tab-bar-select-or-create 1) "Tab 1")
      ("2" (my/tab-bar-select-or-create 2) "Tab 2")
      ("3" (my/tab-bar-select-or-create 3) "Tab 3")
      ("4" (my/tab-bar-select-or-create 4) "Tab 4")
      ("5" (my/tab-bar-select-or-create 5) "Tab 5")
      ("6" (my/tab-bar-select-or-create 6) "Tab 6")
      ("7" (my/tab-bar-select-or-create 7) "Tab 7")
      ("8" (my/tab-bar-select-or-create 8) "Tab 8")
      ("9" (my/tab-bar-select-or-create 9) "Tab 9")
      ("0" (my/tab-bar-select-or-create 10) "Tab 10"))))
  ...
  (pretty-hydra-define hydra-exwm
    ...
    ("Apps"
     (("b" (start-process "epiphany" nil "epiphany")  "browser")
      ("t" (start-process "xfce4-terminal" nil "xfce4-terminal") "terminal")
      ("o" (start-process "libreoffice" nil "libreoffice") "office"))
     "Workspaces"
  ...
  (pretty-hydra-define hydra-dired
    ...
    (""
     (("f" find-name-dired "find file by regexp")
      ("g" find-grep-dired "grep")
      ("d" epa-dired-do-decrypt "decrypt file")
...
(use-package org
  ...
  :custom
  (org-modules '(ol-bbdb ol-bibtex ol-docview ol-doi ol-info))
  ...
  (org-todo-keyword-faces
   '(("TODO"        . (:foreground "LimeGreen"   :weight bold))
     ...
     ("READ"        . (:foreground "IndianRed1"  :weight bold))
...
should be continued."
  ...
  (defface org-checkbox-done
    '((t (:foreground "gray55")))
    "Face for checked checkbox list items.")
  ...
  (font-lock-add-keywords
   ...
   `(("^[ \t]*\\(?:[-+*]\\|[0-9]+[).]\\)[ \t]+\\[X\\][^\n]*"
      0 'org-checkbox-done append))
   'append)
  (set-face-attribute 'org-checkbox-done nil :foreground "gray50" :strike-through nil))
...
(if my/org-journal-directory
    (use-package org-journal
      ...
      (add-to-list 'display-buffer-alist
                   '((major-mode . org-journal-mode)
                     display-buffer-same-window
                     (inhibit-same-window . nil)))

      (defun my/org-journal-jump-tab-bar (&rest args)
        "Jumps to 'Journal' tab."
...
(use-package denote
  ...
  (add-hook 'before-save-hook #'my/denote-rename-on-save-based-on-front-matter))

(message "Notes setup finished")
...
(use-package projectile
  ...
  (defun my/projectile-project-root (orig-fun &optional dir)
    "Disable projectile for remote files."
    (let ((dir (or dir default-directory)))
      (unless (file-remote-p dir)
        (funcall orig-fun dir))))
  (advice-add 'projectile-project-root :around #'my/projectile-project-root)
...
(use-package eglot
  ...
  (add-to-list 'eglot-server-programs '((js-mode js2-mode typescript-ts-mode) . ("rass" "--" "typescript-language-server" "--stdio")))
...
(if (treesit-available-p)
    (use-package treesit
      ...
      :custom
      (setq treesit-font-lock-level 3)  ;; 1–5, higher = more detail
      :config
      ...
      (dolist (src treesit-language-source-alist)
        (unless (treesit-ready-p (car src))
...
(major-mode-hydra-define (prog-mode sgml-mode)
  (:hint nil :color amaranth :quit-key "q" :title (with-fileicon "jsx-2" "Dev" 1 -0.05))
  ("Action"
   (
    ("f" consult-flycheck "flycheck" :exit t)
    ("<left>" flycheck-previous-error "previous error" :exit t)
    ("<right>" flycheck-next-error "next error" :exit t)
...
(if (and (treesit-available-p) (treesit-ready-p 'bash))
...
(custom-set-faces
 ...
 '(org-scheduled ((t (:foreground "#d2b580"))))
 '(org-scheduled-today ((t (:foreground "#d2b580"))))
...
(setq process-adaptive-read-buffering nil)   ; biggest single speedup
(setq shell-file-name (concat (getenv "PREFIX") "/bin/bash"))
(setq explicit-shell-file-name shell-file-name)

(defalias 'sh #'my/new-shell)
...
(use-package term
  ...
  :bind (:map term-raw-map
              ...
              ("C-c 0" . (lambda () (interactive) (my/tab-bar-select-or-create 10))))
  :config
  (defun my/term-handle-exit (&rest _)
  ...
  (defun my/term ()
    "Create a new term."
    (interactive)
    (ansi-term explicit-shell-file-name (generate-new-buffer-name "*ansi-term*"))))

;; (use-package coterm
;;   :straight nil
;;   :commands coterm-mode
;;   :config
;;   (coterm-mode)
...
;;   :straight nil
;;   :commands shell
;;   :defines comint-scroll-to-bottom-on-input
...
;;   :custom
;;   (comint-scroll-to-bottom-on-input 'other)
;;   (comint-scroll-to-bottom-on-output nil)
;;   (comint-prompt-read-only t)
;;   (comint-input-ignoredups t)
;;   (comint-completion-addsuffix t)
;;   (comint-input-ring-size 10000)
;;   (comint-process-echoes t) ;; fix: commands output is duplicated
;;   (comint-terminfo-terminal "ansi")
;;   :config
;;   (defun my/shell-speedup ()
...
;;     (setq-local comint-input-ring-file-name "~/.bash_history")
;;     (comint-read-input-ring 'silent)
;;     (font-lock-mode -1)
;;     (buffer-disable-undo))
;;   (add-hook 'shell-mode-hook 'my/shell-speedup))
...
;;   :config
;;   (setq-default sh-shell-file explicit-shell-file-name)
...
;;   (defun my/new-shell ()
;;     "Always create a new shell buffer."
;;     (interactive)
...
(use-package tramp
  ...
  (tramp-default-method "scp") ;; or rsync
