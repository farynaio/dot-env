...
(use-package woman
  ...
  :config
  (when (my/termux-p)
    (let ((termux-root (getenv "TERMUX__ROOTFS_DIR")))
      (setq woman-manpath
            `(,(expand-file-name "usr/man" termux-root)
              ,(expand-file-name "usr/share/man" termux-root)
              ,(expand-file-name "usr/local/man" termux-root))))))
...
(use-package multiple-cursors
  ...
  (("C->" . my/mc/mark-next-line) ;; might not work on soft keyboard Android
   ("C-<" . my/mc/mark-previous-line) ;; might not work on soft keyboard Android
   ("C-c C-<" . mc/mark-all-like-this)
   :map mc/keymap
   ("<return>" . nil))
  :preface
  (defun my/mc/mark-next-line ()
    "Mark next line using multiple-cursors"
    (interactive)
    (mc/mark-next-lines 1))

  (defun my/mc/mark-previous-line ()
    "Mark previous line using multiple-cursors"
    (interactive)
  ...
  (pretty-hydra-define hydra-multiple-cursors
    (:hint nil :color red :quit-key "q" :title (with-faicon "coffee" "Multiple Cursors" 1 -0.05))
    (""
...
(use-package major-mode-hydra
  ...
  (pretty-hydra-define hydra-eww
    (:hint nil :color teal :quit-key "q" :title (with-faicon "bookmark" "EWW" 1 -0.05))
    (""
     (("i" eww-toggle-images "toggle images")
      ("r" eww-readable "readable" :toggle t)
      ("a" eww-add-bookmark "bookmark")
      ("b" eww-bookmark-jump "goto bookmark")
      ("l" eww-list-bookmarks "bookmarks"))))
...
(use-package org
  ...
  :custom
  (org-modules '(ol-bbdb ol-bibtex ol-docview ol-doi ol-eww ol-info))
  ...
  (defun my/js-lsp-init ()
    ...
    (if (executable-find "typescript-language-server")
        (progn
          ...
          (eglot-ensure))
      (warn "typescript-language-server is not installed! TypeScript language server not available!")))
...
(use-package eww
  ...
  (eww-disable-colorization t)
  (shr-discard-aria-hidden t)
  (shr-use-colors nil)
  (shr-inhibit-images t)  ;; Disable all images. Use eww-toggle-images to toggle
  ;; (shr-blocked-images ".*")
  (shr-max-image-proportion 0.6)
  (eww-cache-removed-data nil)
  (URL-cache-directory "~/.emacs.d/url-cache/")
  (URL-cookie-trusted-urls '()) ;; reject all cookies
...
(use-package eshell
  ...
  (defun my/eshell-prompt ()
    (let* ((pwd (eshell/pwd))
           (pwd (string-remove-prefix (getenv "TERMUX__HOME") pwd))
           (git-branch (magit-get-current-branch))
           (git-branch (if git-branch (propertize (concat " (" git-branch ") " ) 'face '(:foreground "#c475f0")) ""))
           (pwd (concat pwd git-branch "$ ")))
      pwd))
